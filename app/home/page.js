...
const page = () => {
    const { isLoading, isError, data, error } = useQuery({
        queryKey: ['movies'],
        queryFn: ()=> fetch('https://api.themoviedb.org/3/movie/now_playing?api_key=1f5086bec29ee6ee4a7f31e2eee9d13e&language=en-US&page=1&region=IN').then((res)=>res.json()),
    })
    ...
    <div className='px-3 md:px-10 md:py-10 py-5 w-full h-full grid md:grid-cols-4 grid-cols-2 md:gap-6 gap-3'>
        {
          movieList.map((movie,index)=>{
            return (
            <div className='w-full md:h-96 h-60 border-2 border-blue-300 hover:scale-105 hover:duration-300 hover:ease-in-out' key={index}>
              <div className='flex flex-col  rounded-lg border-1 w-full h-full cursor-pointer'>
              <div className='w-full h-3/4'>
              { movieList && 
               <div alt='No-Image' className='w-full h-full bg-cover duration-500' style={{backgroundImage:`url(${movie.images})`}}>
               </div> 
              }  
              </div>
